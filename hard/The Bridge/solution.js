...
const bikes = Array.from({ length: M }, () => ({ x: null, y: null, a: false }));

const roadMap = new Array(4);
for (let i = 0; i < 4; i++) roadMap[i] = readline(); // L0 to L3 are lanes of the road. A dot character . represents a safe space, a zero 0 represents a hole in the road.
...
while (true) {
  ...
  if (count === 0) {
    const state = { speed: S, bikes: bikes, commands: [] };
    dfs(state);
  }
...
function dfs(state) {
  if (isFinished(state)) {
    ...
    if (isBetterThan(candidate, bestPath)) bestPath = candidate;
    return;
  }

  if (isFailed(state) || aliveCount(state) < V) return;
  ...
  for (const cmd of COMMANDS) {
    if (
      (cmd === "UP" || cmd === "DOWN") &&
      !state.bikes.every((b) => canChangeLane(b, cmd))
    )
      continue;

    const next = applyCommand(state, cmd);
    if (aliveCount(next) >= V) dfs(next);
  }
}

function applyCommand(state, cmd) {
  var nextBikes = state.bikes.map((b) => clone(b));
...
function move(bike, speed) {
  bike.a = isGoodRoad(bike, speed);
  bike.x += speed;
}

function isGoodRoad(bike, speed) {
...
function changeLane(bike, speed, cmd) {
  const dir = cmd === "UP" ? -1 : cmd === "DOWN" ? 1 : 0;
  const targetY = bike.y + dir;
  bike.a = isGoodToChangeLane(bike, speed, targetY);
  bike.x += speed;
  bike.y = targetY;
}

function isGoodToChangeLane(bike, speed, targetY) {
  for (let i = bike.x + 1; i < bike.x + speed; i++) {
    if (
...
function canChangeLane(bike, cmd) {
  return (
