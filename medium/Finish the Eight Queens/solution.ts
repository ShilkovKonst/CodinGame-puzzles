const N: number = 8;

const board: string[][] = new Array(N);
const rows: boolean[] = new Array(N).fill(false);
const cols: boolean[] = new Array(N).fill(false);
const leftDiagonal: boolean[] = new Array(2 * N - 1).fill(false);
const rightDiagonal: boolean[] = new Array(2 * N - 1).fill(false);

let queens: number = 0;

for (let i = 0; i < N; i++) {
    const row: string = readline();
    board[i] = row.split("");
    for (let j = 0; j < N; j++) {
        if (board[i][j] === "Q") {
            refreshData(i, j, 1, true);
        }
    }
}

if (placeQueens(0))
    for (let i = 0; i < N; i++) {
        console.log(board[i].join(""));
    }

function placeQueens(row: number): boolean {
    if (queens === N) return true;
    if (row >= N) return false;

    if (rows[row]) {
        return placeQueens(row + 1);
    }

    for (let col = 0; col < N; col++) {
        if (board[row][col] === "." &&
            !rows[row] &&
            !cols[col] &&
            !leftDiagonal[row - col + (N - 1)] &&
            !rightDiagonal[row + col]
        ) {
            refreshData(row, col, 1, true);
            board[row][col] = "Q";

            if (placeQueens(row + 1))
                return true;

            refreshData(row, col, -1, false);
            board[row][col] = ".";
        }
    }
    return false;
}

function refreshData(row: number, col: number, i: number, b: boolean): void {
    queens += i;
    rows[row] = b;
    cols[col] = b;
    leftDiagonal[row - col + (N - 1)] = b;
    rightDiagonal[row + col] = b;
}